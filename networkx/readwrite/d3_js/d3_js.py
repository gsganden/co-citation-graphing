...
def d3_json(G, group=None):
	...
	if group is None:
		...
		try:
			graph_json = {'nodes' : map(lambda n: {'name': str(re.search(r'(\S+)\s.+?(\d\d\d\d)', node_labels[n][1]).group(1)) + ' ' + str(re.search(r'(\S)\s.+?(\d\d\d\d)', node_labels[n][1]).group(2)), 'group' : graph_nodes[n][1][group]}, xrange(len(node_labels)))}
		except KeyError:
